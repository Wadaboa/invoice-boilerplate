...
\begin{spreadtab}{{tabular}[t t t]{lp{8.2cm}r}}
  ...
  $if(VAT)$
    @ & @ \multicolumn{1}{r}{Subtotale:}                & :={sum(c1:[0,-1])} \\ \hhline{~~-}
    @ & @ \multicolumn{1}{r}{Ritenuta d'acconto $VAT$\%:}               & :={$if(net-prices)$(sum(c1:[0,-2]))/(1-($VAT$/100))-sum(c1:[0,-2])$else$($VAT$)/100*[0,-1]$endif$} \\ \hhline{~~-}
  $endif$
  @ & @ \multicolumn{1}{r}{\textbf{Totale:}}   & \textbf{:={$if(VAT)$[0,-1]+[0,-2]$else$sum(c1:[0,-1])$endif$}} \\ \hhline{~~-}
\end{spreadtab}

...
{
  pdfauthor={$author$},
  pdfsubject=Ricevuta \#$invoice-nr$ del $invoice-date$,
  pdftitle=Ricevuta \#$invoice-nr$ del $invoice-date$,
  linkcolor=blue,
  citecolor=blue,
  filecolor=black,
  urlcolor=blue
...
\section*{\textsc{Ricevuta} \textsc{\#$invoice-nr$} del $invoice-date$}
\footnotesize
\newcounter{pos}
\setcounter{pos}{0}
...
\begin{spreadtab}{{tabular}[t t t]{lp{8.2cm}r}}
  ...
  $if(VAT)$
    @ & @ \multicolumn{1}{r}{Totale lordo:}                & :={sum(c1:[0,-1])} \\ \hhline{~~-}
    @ & @ \multicolumn{1}{r}{Ritenuta d'acconto $VAT$\%:}               & :={($VAT$)/100*[0,-1]} \\ \hhline{~~-}
  $endif$
  @ & @ \multicolumn{1}{r}{\textbf{Totale netto (da pagare):}}   & \textbf{:={$if(VAT)$ [0,-2]-[0,-1] $else$ sum(c1:[0,-1]) $endif$}} \\ \hhline{~~-}
\end{spreadtab}
